...
Actions are like messages or requests that get dispatched to your app, then the reducers process those messages and return an updated store. that's why the store is a single source of truth. state is no longer managed in individual components like it is in react.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state lives in a store and is updated when actions are triggered. Component state lives in the component. You could use both in the same app. the store is better to use if multiple components will be accessing and updated the data. 

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

redux thunk is middleware and it allows us to create async action creators to get data from any API
...
- [ yar] **RUN** `yarn` to retrieve all `server-side` the dependencies.
...
- [xsprint ] **Open** `src/index.js` to make sure that your app is ready to roll with the proper middleware.
